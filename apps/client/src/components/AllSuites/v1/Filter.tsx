...
function Filter({ filters, setFilters }: Props) {
  ...
  const inputBase = `w-full px-3 py-2 rounded-lg text-sm outline-none transition-all duration-150
  ...
  return (
    ...
    >
      ...
      >
        <SlidersHorizontal size={14} className={`${themes.text.secondary}`} />
        <span
          className={`text-xs font-semibold uppercase tracking-widest ${themes.text.secondary}`}
        >
          Filters
        </span>
      </div>
      ...
      <div className="p-4 space-y-3">
        <div className="relative">
          <Search
            size={14}
            className={`absolute left-3 top-1/2 -translate-y-1/2 ${themes.text.secondary} pointer-events-none`}
          />
          <input
            type="text"
        ...
        <div>
          <label
            className={`block text-xs font-medium mb-1.5 ${themes.text.secondary}`}
          >
            Status
          </label>
                ...
                className={`flex-1 py-1.5 rounded-lg text-xs font-medium transition-all duration-150 cursor-pointer active:scale-95
                  ${
                    filters.publishStatus === status
                      ? `${themes.background.heroPrimary} ${themes.text.inverted}`
                      : `${themes.background.secondary} ${themes.text.secondary} ${themes.border.fadedThin} hover:opacity-80`
                  }`}
              >
                {status === "ALL"
                  ? "All"
        ...
        <div>
          ...
          <div className="flex gap-2">
            ...
            />
            <div
              className={`flex items-center text-xs ${themes.text.secondary}`}
            >
              →
            </div>
            <input
