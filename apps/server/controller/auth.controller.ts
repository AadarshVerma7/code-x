...
class AuthController {
  async OrganizationRegister(req: Request, res: Response) {
    try {
      ...
      if (existingOrganization)
        throw new Error("Organization with this mail already exists");
  ...
  async InterviewerLogin(req: Request, res: Response) {
    try {
      ...
      return res
        .status(200)
        .json(
          apiResponse(200, "login successful", { data: dbinterviewer, token }),
        );
    } catch (error: any) {
  ...
  async resetPassword(req: Request, res: Response) {
    try {
      ...
      let userType: "INTERVIEWER" | "USER" | "ORGANIZATION" | null = null;
      let dbUser: any = null;
      ...
      if (!dbUser) {
        dbUser = await prismaClient.organization.findFirst({
          where: { email },
        });
